/**
 * Tagentry Plugin JavaScript
 *
 * Original authors: Robin Gareus, Trailjeep
 * Updated by Rob How 2026
 */
...
function tagentry_settag(tagname, on) {
  ...
  if (tagstart >= 0) {
    ...
    if (tagend < 0) {
      alert('incomplete "{{tag>}}" - missing trailing "}}" ?');
      return;
    }
    // remove this tag if already present
